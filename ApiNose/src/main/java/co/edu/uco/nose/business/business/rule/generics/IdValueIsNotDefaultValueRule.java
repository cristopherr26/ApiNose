...
public final class IdValueIsNotDefaultValueRule implements Rule {
	...
	public void execute(final Object... data) {
		...
		if(ObjectHelper.isNull(data)) {
			...
			var technicalMessage = MessagesEnum.TECHNICAL_ERROR_VALUE_UUID_IS_NOT_DEFAULT.getContent();
			throw NoseException.create(userMessage, technicalMessage);
		}
		
		if(data.length < 2) {
			var userMessage = MessagesEnum.USER_ERROR_TRYING_TO_MAKE_AN_OPERATION.getContent();
			var technicalMessage = MessagesEnum.TECHNICAL_ERROR_WRONG_UUID_LENGTH_IS_NOT_DEFAULT.getContent();
			throw NoseException.create(userMessage, technicalMessage);
		}
